...
const TIME_RANGES = [
  { label: '7D', days: 7 }, { label: '2W', days: 14 }, { label: '1M', days: 30 },
...
export default function Transactions() {
  ...
  async function handleSave(data) {
    if (editTx) {
      await window.api.transactions.update(editTx.id, data)
    } else {
      await window.api.transactions.add(data)
    }
    setShowModal(false)
    setEditTx(null)
    loadTxs()
    ...
    <div className="h-full flex flex-col animate-fade-in">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-border shrink-0">
        <div className="flex items-center justify-between mb-4">
          <h1 className="font-display text-2xl font-bold text-text-bright">Transactions</h1>
          <button
            onClick={handleAdd}
...
function TxRow({ tx, categories, currency, onEdit, onDelete }) {
  const cat = categories.find(c => c.name === tx.category)
  const fmt = n => `${Math.abs(n).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`

  return (
    <tr className="group hover:bg-surface-1 transition-colors rounded-xl">
...
function TxModal({ tx, categories, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-surface-1 border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl animate-slide-up">
        <h2 className="font-display text-xl font-bold text-text-bright mb-5">{tx ? 'Edit' : 'Add'} Transaction</h2>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-border text-muted hover:text-text text-sm transition-all">Cancel</button>
